...
async def cmd_food(message: Message, state: FSMContext):
    await message.answer(
        ...
        reply_markup=KeyboardGenerator.generate_one_row_reply_markup_keyboard(
            CmdFoodData.available_food_names
        ),
    )
...
@router.message(
    OrderFood.choosing_food_name, F.text.in_(CmdFoodData.available_food_names)
)
async def food_chosen(message: Message, state: FSMContext):
    ...
    await message.answer(
        ...
        reply_markup=KeyboardGenerator.generate_one_row_reply_markup_keyboard(
            CmdFoodData.available_food_sizes
        ),
    )
...
@router.message(
    OrderFood.choosing_food_size, F.text.in_(CmdFoodData.available_food_sizes)
)
async def food_size_chosen(message: Message, state: FSMContext):
    user_data = await state.get_data()
    await message.answer(
        text=f"You've been chosen {message.text.lower()} size: {user_data['chosen_food']}.\n"
...
async def cmd_cancel(message: Message, state: FSMContext):
    await state.clear()
    await message.answer(text="Action has canceled", reply_markup=ReplyKeyboardRemove())
